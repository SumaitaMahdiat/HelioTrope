...
function buildDemoReply(prompt) {
    ...
    if (p.includes("eid")) {
        return ("For an Eid dinner, go for a polished but comfortable look: a black or deep-navy base with one warm accent (gold/tan). " +
    ...
    if (p.includes("black jeans") || p.includes("black jean")) {
        return ("Black jeans are super flexible: try a white/cream top for contrast, then add a neutral shoe and one accent (gold bag/earrings). " +
            "For a dressier vibe, add an outer layer like a blazer or long shrug.");
    }
    return ("Tell me the occasion, your comfort level (modest/regular), and 3–5 items you own (colors + type). " +
