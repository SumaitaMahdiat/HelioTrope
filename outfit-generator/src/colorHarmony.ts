...
function family(color: string): "warm" | "cool" | "neutral" | "other" {
  const c = color.toLowerCase().trim();
  if (NEUTRAL.has(c)) return "neutral";
  if (WARM.has(c)) return "warm";
  if (COOL.has(c)) return "cool";
  return "other";
}
...
export function colorHarmonyScore(colors: string[]): number {
  ...
  if (nonNeutral.length <= 1) return 0.85 + neutrals * 0.02;

  const warmCount = nonNeutral.filter((f) => f === "warm").length;
  const coolCount = nonNeutral.filter((f) => f === "cool").length;
  if (warmCount > 0 && coolCount > 0) return 0.55;

  const uniq = new Set(nonNeutral);
  if (uniq.size === 1) return 0.9;

  return 0.72;
}

export function harmonyNote(colors: string[]): string {
  const score = colorHarmonyScore(colors);
  if (score >= 0.85) return "Colors read cohesive (neutrals or same family).";
  if (score >= 0.7) return "Moderate harmony; consider swapping one accent for a neutral.";
