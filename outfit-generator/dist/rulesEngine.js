...
function occasionMatch(item, occasion) {
    if (!occasion)
        return 0;
...
function cluelessStyleBonus(outfit) {
    let score = 0;
    const notes = [];
 ...
 */
export function generateOutfitCombinations(closet, occasion, maxOutfits = 12, style = "classic") {
                ...
                for (const full of withExtra) {
                    ...
                    if (style === "clueless") {
                        ...
                        const neutralCount = full.filter((i) => [
                            ...
                            "gray",
                            "grey",
                        ].includes(normalizeColor(i.color))).length;
                        if (neutralCount >= 2) {
                            score += 6;
                            notes.push("Neutral anchor palette keeps the outfit preppy and clean.");
                        }
                    }
