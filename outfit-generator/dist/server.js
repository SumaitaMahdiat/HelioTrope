...
const VALID_STYLES = new Set(["classic", "clueless"]);
const SAMPLE_CLOSET = [
    {
        id: "s1",
...
function validateCloset(closet) {
...
app.post("/api/outfits/generate", async (req, res) => {
    try {
        ...
        const outfits = generateOutfitCombinations(body.closet, body.occasion, 15, body.style ?? "classic");
        const top = outfits[0] ?? null;
        const key = process.env.OPENROUTER_API_KEY;
        let aiExplanation;
        if (body.useAI && key && top) {
            try {
                aiExplanation = await explainOutfitWithAI(top, body.occasion, key, body.closetImages);
            }
            catch (e) {
